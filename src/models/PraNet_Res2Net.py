# This code is from https://github.com/DengPingFan/PraNet/blob/master/lib/PraNet_Res2Net.py
...
class AuxRegressor(nn.Module):
    def __init__(self):
        super(AuxRegressor, self).__init__()
        self.upsample_2 = nn.Upsample(
            scale_factor=2, mode="bilinear", align_corners=True
        )
        self.conv1 = nn.Conv2d(1536, 256, 1, 1)
        self.conv2 = nn.Conv2d(512, 1, 1, 1)
        self.relu = nn.ReLU()

    def forward(self, v1, v2, v3):
        v3 = self.upsample_2(v3)
        v = torch.cat([v3, v2], dim=1)
        v = self.conv1(v)
        v = self.relu(v)
        v = self.upsample_2(v)
...
class PraNet(nn.Module):
    ...
    def __init__(self, channel=32, in_channel=3):
        super(PraNet, self).__init__()
        # ---- ResNet Backbone ----
        self.resnet = res2net50_v1b_26w_4s(pretrained=False, in_channel=in_channel)
        ...
        # >>> aux regressor
        self.aux_regressor = AuxRegressor()
    ...
    def forward(self, x):
        ...
        v2 = x2.clone()
        v3 = x3.clone()
        aux_v = self.aux_regressor(v1, v2, v3)
...
def get_model(resume=None, in_channel=3):
    model = PraNet(in_channel=in_channel)
    changed_keys = ["ra4_conv5", "ra3_conv4", "ra2_conv4", "agg1.conv5"]
    remove_keys = []
    if resume == None:
        resume = "./models/PraNet-19.pth"
    ...
    if in_channel == 1:
        for key in state_dict:
            if "resnet.conv1.0.weight" in key:
                state_dict[key] = state_dict[key][:, 0:1, :, :]

    model.load_state_dict(state_dict, strict=False)  # FIXME
    return model
